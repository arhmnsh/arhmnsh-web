...
function onPageKeydown(event: KeyboardEvent) {
  ...
  page.value = event.key === 'ArrowLeft' ? 'details' : 'take'
  const controls = event.currentTarget as HTMLElement
  controls.querySelector<HTMLButtonElement>(`[data-page="${page.value}"]`)?.focus()
}
...
async function move(from: string, to: string) {
  ...
  const movement = travel.value.animate([{transform:from},{transform:to}], {duration:520,easing:'cubic-bezier(.22,.7,.2,1)',fill:'both'})
  animation = movement
...
<template>
  <div class="book-studio" :style="{ '--binding': appearance.color, '--shelf-lean': `${appearance.lean}deg` }">
    ...
    <div class="reading-controls" :class="{ 'controls-hidden': !controlsReady }">
      <nav class="page-controls" aria-label="Book pages" @keydown="onPageKeydown"><button type="button" data-page="details" :aria-pressed="page === 'details'" @click="page = 'details'"><ChevronLeft :size="14" aria-hidden="true" />Book details</button><button type="button" data-page="take" :aria-pressed="page === 'take'" @click="page = 'take'">My take<ChevronRight :size="14" aria-hidden="true" /></button></nav>
    </div>
  </div>
</template>
...
.reading-book { position:relative; width:100%; height:500px; transform-style:preserve-3d; transform:translateX(-25%) rotateX(3deg) rotateY(-22deg) rotateZ(var(--shelf-lean)); transition:transform 520ms cubic-bezier(.22,.7,.2,1); }
...
.page-body { overflow-y:auto; overscroll-behavior:contain; margin:auto 0; padding:20px 0; }
...
.hinged-cover { position:absolute; left:50%; width:50%; height:100%; transform-style:preserve-3d; transform-origin:0 50%; transform:translateZ(7px) rotateY(0deg); transition:transform 520ms cubic-bezier(.26,.62,.2,1); }
...
.title-body { min-height:0; overflow-y:auto; overscroll-behavior:contain; margin:auto 0; padding:24px 0; text-align:center; }
...
.find-book { display:flex; align-items:center; justify-content:center; gap:7px; min-height:44px; font-size:12px; text-decoration:underline; text-underline-offset:4px; }
.purchase-caption { display:block; margin-top:10px; color:#8f8066; font-size:10px; }
...
.reading-controls button { display:flex; align-items:center; gap:7px; min-height:44px; padding:0 10px; font-size:12px; border:1px solid rgb(255 255 255 / 20%); border-radius:999px; }
.reading-controls button:focus-visible,.find-book:focus-visible { outline:2px solid currentColor; outline-offset:3px; }
